...
utilizada. Este laboratório em especial utiliza três tipos de cobaias:
sapos, ratos e coelhos.

Para obter estas informações, ela sabe exatamente o número de experimentos
que foram realizados, o tipo de cobaia utilizada e a quantidade de cobaias
utilizadas em cada experimento.

Entrada:
A primeira linha de entrada contém um valor inteiro N que indica os vários
casos de teste que vem a seguir. Cada caso de teste contém um inteiro que
representa a quantidade de cobaias utilizadas e um caractere Tipo
('C', 'R' ou 'S'), indicando o tipo de cobaia
...
int main() {
    int N, total = 0, coelho = 0, rato = 0, sapo = 0, qtd; 
    float p_coelho, p_rato, p_sapo;
    char verifica;
    
    scanf("%d", &N);
    
    for (int i = 0; i<N; i++) {
        scanf("%d %c", &qtd, &verifica);
        if (verifica == 'C') {
            coelho += qtd;
            total += qtd;
        }
    ...
    p_coelho = (float)(coelho*100)/(total*1.0) ;
    p_rato = (float)(rato*100)/(total*1.0); 
    p_sapo = (float)(sapo*100)/(total*1.0); 
    
    printf("Total: %d cobaias\nTotal de coelhos: %d\nTotal de ratos: %d\nTotal de sapos: %d\n", total, coelho, rato, sapo);
    printf("Percentual de coelhos: %.2f %\nPercentual de ratos: %.2f %\nPercentual de sapos: %.2f %\n", p_coelho, p_rato, p_sapo);
    return 0;
}
